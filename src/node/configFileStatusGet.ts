import {TranscoderNodeAPI, ConfigFileStatusResponse, ConfigFile} from '../api'
...
export default async function configFileStatusGet(axios: TypedAxiosInstance<TranscoderNodeAPI>): Promise<Either<NetworkError, ConfigFileStatus>> {
    try{
...
import {toDictUsingUniqueKey} from '../utils'


function reshape(resp: ConfigFileStatusResponse): ConfigFileStatus {
    return ConfigFileStatus({
        channels: toDictUsingUniqueKey(resp.channels.slice(0, resp.max_channels)
            .map(ch => ({...ch,
